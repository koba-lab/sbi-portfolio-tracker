...
- **Git Flow**: mainとdevelopブランチを分離、リリースブランチ管理
- **GitHub Flow**: mainブランチのみ、featureブランチからPR
- **Trunk-Based Development**: mainへ直接コミット、短命ブランチ
...
1. **シンプルさ**: ブランチ戦略が単純明快
   - mainブランチのみ保護
   - featureブランチから直接PR
   - リリースブランチ不要（CI/CDで自動デプロイ）

2. **AIとの協調開発に最適**:
   - featureブランチでAIに広範な権限付与
   - mainブランチは保護（直接コミット禁止）
   - PR単位でレビュー・承認フロー
...
4. **個人開発規模に適切**:
   - Git Flowは過剰（develop/release不要）
   - Trunk-Basedは保護が弱い（AI暴走リスク）
...
- AIへの権限を多めに付与
  ...
  - PR作成
  - CI失敗時の自動修正
- ブランチ命名規則: `feature/機能名`, `fix/バグ名`, `docs/ドキュメント名`

**PR運用**:
- PRテンプレート使用（概要・実装内容・Next Tasks）
- CI通過を必須条件
- レビュー承認後にマージ
- マージ後はfeatureブランチ削除
...
- **複雑すぎる**: develop/release/hotfixブランチ管理が煩雑
- **個人開発には過剰**: チーム開発での厳格なリリース管理が不要
- **AI連携が困難**: ブランチ切り替えロジックが複雑化

#### Trunk-Basedを採用しなかった理由

- **保護が不十分**: mainへ直接コミット可能だとAI暴走のリスク
- **レビュー機会の喪失**: PR単位でのレビューが望ましい
- **ロールバックが煩雑**: コミット単位でrevertが必要

### 議論の経緯

「AIに権限を渡しつつ、mainブランチは保護したい」という要件から、featureブランチでのみAIを動かし、PRでレビューするGitHub Flowが最適と判断。
